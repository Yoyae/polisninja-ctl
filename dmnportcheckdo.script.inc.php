...
//  $version = $config[$testnet]['Version'];
  ...
  try
  {
    $c = new \Polis\Node($ip,$port,DMN_PORTCHECK_TIMEOUT,DMN_VERSION,$sversion,$protocol,$magic);
    $subver = $c->getSubVer();
    $c->closeConnection();
    $res = 1;
  }
  catch (\Polis\EFailedToReadFromPeer $eftrfp) {
    $subver = '';
    $errmsg = $eftrfp->getMessage();
    $res = 1;
  }
  catch (\Polis\EUnexpectedFragmentation $euf) {
    $subver = '';
    $errmsg = $euf->getMessage();
    $res = 1;
  }
  catch (\Polis\EUnexpectedPacketType $eupt) {
    $subver = '';
