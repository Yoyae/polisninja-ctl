...
function dmn_getip($pid,$uname) {
  ...
  $res = false;
  exec('netstat -ntpl | grep "tcp  " | egrep ":(1)?24126" | grep "'.$pid.'/darkcoind\|'.$pid.'/polisd"',$output,$retval);
  if (isset($output[0])) {
...
function dmn_getstatus($polisdinfo,$blockhash) {
  ...
  if ($polisdinfo !== false) {
    if (array_key_exists('version',$polisdinfo)) {
      $res['version'] = $polisdinfo['version'];
    }
    if (array_key_exists('protocolversion',$polisdinfo)) {
      $res['protocol'] = $polisdinfo['protocolversion'];
    }
    if (array_key_exists('difficulty',$polisdinfo)) {
      $res['difficulty'] = $polisdinfo['difficulty'];
    }
    if (array_key_exists('blocks',$polisdinfo)) {
      $res['blocks'] = $polisdinfo['blocks'];
    }
    if (array_key_exists('connections',$polisdinfo)) {
      $res['connections'] = $polisdinfo['connections'];
    }
    if (array_key_exists('testnet',$polisdinfo)
     && $polisdinfo['testnet']) {
      $res['testnet'] = 1;
    }
    $res['encryptedwallet'] = array_key_exists('unlocked_until',$polisdinfo);
  }
  return $res;
...
// Create a new Polis Masternode user, prepare folder and configuration
function dmn_create($dmnpid,$ip,$forcename = '') {
  ...
  exec('useradd -m -c "Polis$testinfo MasterNode #'.$newnum.'" -U -s /bin/false -p '.randomPassword(128).' '.$newuname.' 1>/dev/null 2>/dev/null',$output,$retval);
  if ($retval != 0) {
    echo "Already exists!\n";
    if ($forcename == '') {
      die;
  ...
  echo "Generating polis.conf";
  mkdir("/home/$newuname/.poliscore");
  touch("/home/$newuname/.poliscore/polis.conf");
  chmod("/home/$newuname/.poliscore",0700);
  chmod("/home/$newuname/.poliscore/polis.conf",0600);
  $conflist = array('server=1',
         'rpcuser='.$newuname.'rpc',
         'rpcpassword='.randomPassword(128),
         'alertnotify=echo %s | mail -s "Polis MasterNode #'.str_pad($newnum,2,'0',STR_PAD_LEFT).' Alert" somebody@mowhere.blackhole',
         'rpcallowip=127.0.0.1',
         "bind=$ip",
         'rpcport='.(intval($newnum)+DMNCTLRPCPORTVAL).'21426',
         'masternode=0',
         "externalip=$ip",
         '#mnctlcfg#enable=1');
  if (DMNTESTNET === true) {
  ...
  $polisconf = implode("\n",$conflist);
  file_put_contents("/home/$newuname/.poliscore/polis.conf",$polisconf);
  echo "OK\n";
  echo "Setting ACL";
  ...
  chown("/home/$newuname/.poliscore/",$newuname);
  chgrp("/home/$newuname/.poliscore/",$newuname);
  chown("/home/$newuname/.poliscore/polis.conf",$newuname);
  chgrp("/home/$newuname/.poliscore/polis.conf",$newuname);
  echo "OK\n";
...
// Set the enable flag to 0 in polis.conf to disable the Masternode
function dmn_disable($dmnpid,$dmntodisable) {
  foreach ($dmntodisable as $uname) {
    ...
    if (dmn_finduname($dmnpid,$uname)) {
      ...
      echo "Unknown Polis MasterNode";
    }
    echo "\n";
  }
}

// Set the enable flag to 1 in polis.conf to enable the Masternode
...
function dmn_startstop($dmnpid,$todo,$testnet = false,$nodetype = 'masternode',$withreindex = false) {
  ...
  foreach($nodes as $nodenum => $node) {
    ...
    $commands[] = array("status" => 0,
                        "nodenum" => $nodenum,
                        "cmd" => "$uname $todo ".$node['polisd'].$extra,
                        "exitcode" => -1,
                        "output" => '');
  }
...
function dmn_status($dmnpid,$istestnet) {
  ...
  foreach($dmnpid as $dmnnum => $dmnpidinfo) {
    ...
    if (($dmnpidinfo['pid'] !== false) && ($dmnpidinfo['currentbin'] != '')) {
      ...
      // Parse status
      $polisdinfo = dmn_getstatus($dmnpidinfo['info'],$dmnpidinfo['blockhash']);
      $blocks = $polisdinfo['blocks'];
      $blockhash = $polisdinfo['blockhash'];
      $connections = $polisdinfo['connections'];
      $difficulty = $polisdinfo['difficulty'];
      $protocol = $polisdinfo['protocol'];
      $version = $polisdinfo['version'];
      ...
      if ($version !== false) {
        ...
        if ($dmnpidinfo['versionhandling'] <= 2) {
          ...
          $mncurrentlist[$uname] = $mncurrentip.":".$polisdinfo['testnet'];
          foreach($dmnpidinfo['mnlastseen'] as $mnlsip => $data) {
            $mnlastseen[$uname][$mnlsip.':'.$polisdinfo['testnet']] = $data;
          }
          foreach($dmnpidinfo['mnactiveseconds'] as $mnlsip => $data) {
            $mnactivesince[$uname][$mnlsip.':'.$polisdinfo['testnet']] = $data;
          }
          ...
          foreach($mnlist as $ip => $activetrue) {
            ...
            $mnlistfinal["$ip:".$polisdinfo['testnet']][$uname] = array('Status' => $active,
                                                                           'PoS' => $mnpose[$ip],
                                                                           'StatusEx' => $activetrue);
          }
          if (is_array($mnvoteslist) && (count($mnvoteslist)>0)) {
            foreach($mnvoteslist as $ip => $vote) {
              $mnvoteslistfinal["$ip:".$polisdinfo['testnet']][$uname] = $vote;
            }
          }
          foreach($mnpubkeylist as $data) {
            $mnpubkeylistfinal[$data["ip"].":".$data["port"].":".$polisdinfo['testnet'].":".$data["pubkey"]] = array(
                     "MasternodeIP" => $data["ip"],
                     "MasternodePort" => $data["port"],
                     "MNTestNet" => $polisdinfo['testnet'],
                     "MNPubKey" => $data["pubkey"]
                );
          }
          if (is_array($mndonationlist)) {
            foreach($mndonationlist as $donatedata) {
              $mndonationlistfinal[$donatedata["ip"].":".$donatedata["port"].":".$polisdinfo['testnet'].":".$donatedata["pubkey"]] = array(
        ...
        elseif ($dmnpidinfo['versionhandling'] >= 3) {
          ...
          if ($dmnpidinfo['versionhandling'] == 3) {
              ...
              if (is_array($dmnpidinfo['mnbudgetshow'])) {
                  foreach ($dmnpidinfo['mnbudgetshow'] as $mnbudgetid => $mnbudgetdata) {
                      if (array_key_exists($polisdinfo['testnet'] . "-" . $mnbudgetdata["Hash"], $mnbudgetshow)) {
                          if (($mnbudgetshow[$polisdinfo['testnet'] . "-" . $mnbudgetdata["Hash"]]["Yeas"]
                                  + $mnbudgetshow[$polisdinfo['testnet'] . "-" . $mnbudgetdata["Hash"]]["Nays"]
                                  + $mnbudgetshow[$polisdinfo['testnet'] . "-" . $mnbudgetdata["Hash"]]["Abstains"]) < ($mnbudgetdata["Yeas"] + $mnbudgetdata["Nays"] + $mnbudgetdata["Abstains"])
                          ) {
                              $mnbudgetshow[$polisdinfo['testnet'] . "-" . $mnbudgetdata["Hash"]] = $mnbudgetdata;
                              $mnbudgetshow[$polisdinfo['testnet'] . "-" . $mnbudgetdata["Hash"]]['BudgetId'] = $mnbudgetid;
                              $mnbudgetshow[$polisdinfo['testnet'] . "-" . $mnbudgetdata["Hash"]]["BudgetTesnet"] = $polisdinfo['testnet'];
                          }
                      } else {
                          $mnbudgetshow[$polisdinfo['testnet'] . "-" . $mnbudgetdata["Hash"]] = $mnbudgetdata;
                          $mnbudgetshow[$polisdinfo['testnet'] . "-" . $mnbudgetdata["Hash"]]['BudgetId'] = $mnbudgetid;
                      ...
                      if (array_key_exists("mnbudget-getvotes-" . $mnbudgetid, $dmnpidinfo)) {
                          if (!array_key_exists($mnbudgetid, $mnbudgetvotes[$polisdinfo['testnet']])) {
                              $mnbudgetvotes[$polisdinfo['testnet']][$mnbudgetid] = array();
                          }
                          if (is_array($dmnpidinfo["mnbudget-getvotes-" . $mnbudgetid])) {
                              foreach ($dmnpidinfo["mnbudget-getvotes-" . $mnbudgetid] as $mnbudgetvotehash => $mnbudgetvotedata) {
                                  if (array_key_exists($mnbudgetvotehash, $mnbudgetvotes[$polisdinfo['testnet']][$mnbudgetid])) {
                                      if ($mnbudgetvotes[$polisdinfo['testnet']][$mnbudgetid][$mnbudgetvotehash]["nTime"] < $mnbudgetvotedata["nTime"]) {
                                          $mnbudgetvotes[$polisdinfo['testnet']][$mnbudgetid][$mnbudgetvotehash] = $mnbudgetvotedata;
                                      }
                                  } else {
                                      $mnbudgetvotes[$polisdinfo['testnet']][$mnbudgetid][$mnbudgetvotehash] = $mnbudgetvotedata;
                                  }
              ...
              if (is_array($dmnpidinfo['mnbudgetprojection'])) {
                  foreach ($dmnpidinfo['mnbudgetprojection'] as $mnbudgetid => $mnbudgetdata) {
                      if (is_array($mnbudgetdata) && array_key_exists("Yeas", $mnbudgetdata) && array_key_exists("Nays", $mnbudgetdata) && array_key_exists("Abstains", $mnbudgetdata)) {
                          if (array_key_exists($mnbudgetdata["Hash"], $mnbudgetprojection[$polisdinfo['testnet']])) {
                              if (($mnbudgetprojection[$polisdinfo['testnet']][$mnbudgetdata["Hash"]]["Yeas"]
                                      + $mnbudgetprojection[$polisdinfo['testnet']][$mnbudgetdata["Hash"]]["Nays"]
                                      + $mnbudgetprojection[$polisdinfo['testnet']][$mnbudgetdata["Hash"]]["Abstains"]) < ($mnbudgetdata["Yeas"] + $mnbudgetdata["Nays"] + $mnbudgetdata["Abstains"])
                              ) {
                                  $mnbudgetprojection[$polisdinfo['testnet']][$mnbudgetdata["Hash"]] = $mnbudgetdata;
                                  $mnbudgetprojection[$polisdinfo['testnet']][$mnbudgetdata["Hash"]]['BudgetId'] = $mnbudgetid;
                                  $mnbudgetprojection[$polisdinfo['testnet']][$mnbudgetdata["Hash"]]["BudgetTesnet"] = $polisdinfo['testnet'];
                              }
                          } else {
                              $mnbudgetprojection[$polisdinfo['testnet']][$mnbudgetdata["Hash"]] = $mnbudgetdata;
                              $mnbudgetprojection[$polisdinfo['testnet']][$mnbudgetdata["Hash"]]['BudgetId'] = $mnbudgetid;
              ...
              if (is_array($dmnpidinfo['mnbudgetfinal'])) {
                  foreach ($dmnpidinfo['mnbudgetfinal'] as $mnbudgetid => $mnbudgetdata) {
                      if (array_key_exists($polisdinfo['testnet'] . "-" . $mnbudgetdata["Hash"], $mnbudgetfinal) &&
                          array_key_exists("VoteCount", $mnbudgetfinal[$polisdinfo['testnet'] . "-" . $mnbudgetdata["Hash"]])
                      ) {
                          if (($mnbudgetfinal[$polisdinfo['testnet'] . "-" . $mnbudgetdata["Hash"]]["VoteCount"]) < ($mnbudgetdata["VoteCount"])) {
                              $mnbudgetfinal[$polisdinfo['testnet'] . "-" . $mnbudgetdata["Hash"]] = $mnbudgetdata;
                              $mnbudgetfinal[$polisdinfo['testnet'] . "-" . $mnbudgetdata["Hash"]]['BudgetName'] = $mnbudgetid;
                              $mnbudgetfinal[$polisdinfo['testnet'] . "-" . $mnbudgetdata["Hash"]]["BudgetTesnet"] = $polisdinfo['testnet'];
                          }
                      } else {
                          $mnbudgetfinal[$polisdinfo['testnet'] . "-" . $mnbudgetdata["Hash"]] = $mnbudgetdata;
                          $mnbudgetfinal[$polisdinfo['testnet'] . "-" . $mnbudgetdata["Hash"]]['BudgetName'] = $mnbudgetid;
          ...
          elseif ($dmnpidinfo['versionhandling'] == 4) {
              ...
              if (($governancenextsb[$polisdinfo['testnet']] === false) || ($governancenextsb[$polisdinfo['testnet']] > intval($dmnpidinfo['getgovernanceinfo']['nextsuperblock']))) {
                $governancenextsb[$polisdinfo['testnet']] = intval($dmnpidinfo['getgovernanceinfo']['nextsuperblock']);
              }
              // Store the budget available in next superblock
              if (($governancebudget[$polisdinfo['testnet']] === false) || ($governancebudget[$polisdinfo['testnet']] > floatval($dmnpidinfo['getsuperblockbudget']))) {
                $governancebudget[$polisdinfo['testnet']] = floatval($dmnpidinfo['getsuperblockbudget']);
              }
              // Parse proposals
              if (is_array($dmnpidinfo["gobjectlist"]) && is_array($dmnpidinfo["gobjectlist"]["proposals"])) {
                  foreach ($dmnpidinfo["gobjectlist"]["proposals"] as $proposaldata) {
                      if (array_key_exists($polisdinfo['testnet'] . "-" . $proposaldata["hash"], $gobjectproposallist)) {
                          if (($gobjectproposallist[$polisdinfo['testnet'] . "-" . $proposaldata["hash"]]["gobject"]["YesCount"]
                             + $gobjectproposallist[$polisdinfo['testnet'] . "-" . $proposaldata["hash"]]["gobject"]["NoCount"]
                             + $gobjectproposallist[$polisdinfo['testnet'] . "-" . $proposaldata["hash"]]["gobject"]["AbstainCount"]) < ($proposaldata["gobject"]["YesCount"] + $proposaldata["gobject"]["NoCount"] + $proposaldata["gobject"]["AbstainCount"])
                          ) {
                              $gobjectproposallist[$polisdinfo['testnet'] . "-" . $proposaldata["hash"]] = $proposaldata;
                              $gobjectproposallist[$polisdinfo['testnet'] . "-" . $proposaldata["hash"]]["Testnet"] = $polisdinfo['testnet'];
                          }
                      } else {
                          $gobjectproposallist[$polisdinfo['testnet'] . "-" . $proposaldata["hash"]] = $proposaldata;
                          $gobjectproposallist[$polisdinfo['testnet'] . "-" . $proposaldata["hash"]]["Testnet"] = $polisdinfo['testnet'];
                      ...
                      if (array_key_exists("gobject-getvotes-" . $proposaldata["hash"], $dmnpidinfo)) {
                          if (!array_key_exists($proposaldata["hash"], $gobjectvotes[$polisdinfo['testnet']])) {
                              $gobjectvotes[$polisdinfo['testnet']][$proposaldata["hash"]] = array();
                          }
                              ...
                              foreach ($dmnpidinfo["gobject-getvotes-" . $proposaldata["hash"]] as $gobjectvotehash => $gobjectvotedata) {
                                ...
                                if ($signal == "FUNDING") {
                                  if ((substr($collateral,0,16) == "CTxIn(COutPoint(") && (substr($collateral,-14) == "), scriptSig=)")) {
                                      ...
                                      if (array_key_exists($mnoutputhash."-".$mnoutputindex, $gobjectvotes[$polisdinfo['testnet']][$proposaldata["hash"]])) {
                                          if ($gobjectvotes[$polisdinfo['testnet']][$proposaldata["hash"]][$mnoutputhash."-".$mnoutputindex]["nTime"] < $ntime) {
                                              $gobjectvotes[$polisdinfo['testnet']][$proposaldata["hash"]][$mnoutputhash."-".$mnoutputindex] = array("MasternodeOutputHash" => $mnoutputhash,
              ...
              if (is_array($dmnpidinfo["gobjectlist"]) && is_array($dmnpidinfo["gobjectlist"]["triggers"])) {
                  foreach ($dmnpidinfo["gobjectlist"]["triggers"] as $triggerdata) {
                      if (array_key_exists($polisdinfo['testnet'] . "-" . $triggerdata["hash"], $gobjecttriggerlist)) {
                          if (($gobjecttriggerlist[$polisdinfo['testnet'] . "-" . $triggerdata["hash"]]["gobject"]["YesCount"]
                                  + $gobjecttriggerlist[$polisdinfo['testnet'] . "-" . $triggerdata["hash"]]["gobject"]["NoCount"]
                                  + $gobjecttriggerlist[$polisdinfo['testnet'] . "-" . $triggerdata["hash"]]["gobject"]["AbstainCount"]) < ($triggerdata["gobject"]["YesCount"] + $triggerdata["gobject"]["NoCount"] + $triggerdata["gobject"]["AbstainCount"])
                          ) {
                              $gobjecttriggerlist[$polisdinfo['testnet'] . "-" . $triggerdata["hash"]] = $triggerdata;
                              $gobjecttriggerlist[$polisdinfo['testnet'] . "-" . $triggerdata["hash"]]["Testnet"] = $polisdinfo['testnet'];
                      ...
                      if (array_key_exists("gobject-getvotes-" . $triggerdata["hash"], $dmnpidinfo)) {
                          if (!array_key_exists($triggerdata["hash"], $gobjectvotes[$polisdinfo['testnet']])) {
                              $gobjectvotes[$polisdinfo['testnet']][$triggerdata["hash"]] = array();
                          }
                              ...
                              foreach ($dmnpidinfo["gobject-getvotes-" . $triggerdata["hash"]] as $gobjectvotehash => $gobjectvotedata) {
                                  ...
                                  if ($signal == "FUNDING") {
                                      if ((substr($collateral,0,16) == "CTxIn(COutPoint(") && (substr($collateral,-14) == "), scriptSig=)")) {
                                          $collateral = substr($collateral, 16, strlen($collateral) - 30);
                                          list($mnoutputhash, $mnoutputindex) = explode(", ", $collateral);
                                          if (array_key_exists($mnoutputhash."-".$mnoutputindex, $gobjectvotes[$polisdinfo['testnet']][$triggerdata["hash"]])) {
                                              if ($gobjectvotes[$polisdinfo['testnet']][$triggerdata["hash"]][$mnoutputhash."-".$mnoutputindex]["nTime"] < $ntime) {
                                                  $gobjectvotes[$polisdinfo['testnet']][$triggerdata["hash"]][$mnoutputhash."-".$mnoutputindex] = array("MasternodeOutputHash" => $mnoutputhash,
                                                      "MasternodeOutputIndex" => intval($mnoutputindex),
                                                      "VoteHash" => $gobjectvotehash,
                                                      "nTime" => intval($ntime),
                                                      "Vote" => $vote);
          ...
          foreach($mn3listfull as $mn3output => $mn3data) {
            ...
            if (array_key_exists($mn3output."-".$polisdinfo['testnet'],$mninfo2)) {
              if ($mn3lastseen < $mninfo2[$mn3output."-".$polisdinfo['testnet']]["MasternodeLastSeen"]) {
                $mninfo2[$mn3output."-".$polisdinfo['testnet']]["MasternodeLastSeen"] = intval($mn3lastseen);
              }
              if ($mn3activeseconds < $mninfo2[$mn3output."-".$polisdinfo['testnet']]["MasternodeActiveSeconds"]) {
                $mninfo2[$mn3output."-".$polisdinfo['testnet']]["MasternodeActiveSeconds"] = intval($mn3activeseconds);
              }
              if ($mn3lastpaid > $mninfo2[$mn3output."-".$polisdinfo['testnet']]["MasternodeLastPaid"]) {
                $mninfo2[$mn3output."-".$polisdinfo['testnet']]["MasternodeLastPaid"] = intval($mn3lastpaid);
              }
            }
            else {
              $mninfo2[$mn3output."-".$polisdinfo['testnet']] = array("MasternodeProtocol" => intval($mn3protocol),
            ...
            $mnlist2final[$mn3output."-".$polisdinfo['testnet']][$uname] = array('Status' => $active,
                                                                                    'StatusEx' => $mn3status);
          }
        }
        echo " $dmnip\n";
  ...
  if (count($mnlistfinal) > 0) {
    ...
    $estpayoutdaily = round(dmn_getpayout($mncountactive,$polisdinfo['difficulty']),2);
  }
  else {
    $estpayoutdaily = '???';
  }

//  echo "Total Masternodes: $mncount/$mncountinactive    Est.Payout: $estpayoutdaily POLIS/day (diff=$difficultyfinal)\n";
...
xecho("Polis Ninja Control [dmnctl] v".DMN_VERSION." (".date('Y-m-d H:i:s',filemtime(__FILE__)).")\n");
