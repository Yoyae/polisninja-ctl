...
xecho("Fetching from Polis server: ");
...
//    echo "OK\n";
    ...
    $polisdpath = $tdir."/".$folder."/bin/polisd";
    if (($folder === FALSE) || (!file_exists($polisdpath))) {
		echo '\r\n'.$folder;
		echo '\r\n'.$polisdpath;
    ...
    exec("polisd -?",$output,$ret); //exec($polisdpath." -?",$output,$ret);
    if ($ret != 0) {
        echo "ERROR (polisd return code $ret)\n";
        die2(5);
    }
    if (!preg_match('/^polis Core Daemon version v(.+)$/',$output[0],$match)) {
        echo "ERROR (polisd return version do not match regexp '".$output[0]."')\n";
        die2(6);
    };
    $version = $match[1];
    echo "OK (".$version.")\n";
    xecho("Adding new version to database: ");
    rename($polisdpath,"/opt/polisd/0.12/polisd-".$version);
    //exec("/opt/polisninja-ctl/dmnctl version polisd ".$version." 1 1",$output,$ret); 
	exec("/opt/polisninja-ctl/dmnctl version /opt/polisd/0.12/polisd-".$version." ".$version." 1 1",$output,$ret);
    delTree($tdir);
    echo "OK\n";
    xecho("Restarting testnet node: ");
    exec("/opt/polisninja-ctl/dmnctl restart testnet p2pool",$output,$ret);
    if ($ret != 0) {
        echo "ERROR (return code of dmnctl restart was $ret)\n";
        die2(8);
    }
    exec("/opt/polisninja-ctl/dmnctl restart testnet masternode",$output,$ret);
    if ($ret != 0) {
        echo "ERROR (return code of dmnctl restart was $ret)\n";
        die2(8);
    }
