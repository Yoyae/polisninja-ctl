...
// Deal with polis.conf configuration (read/write)
class PolisConfig {

  // Normal polis.conf configuration
  private $config;
  ...
  private function loadconfig() {
    ...
    if (file_exists($this->configfilename)) {
      ...
      $conf = explode("\n",trim($rawconf));
      $magiclen = strlen(PolisConfig::MAGIC);
      for ($x = 0; $x < count($conf); $x++) {
        if ((substr($conf[$x],0,1) == '#') && (substr($conf[$x],0,$magiclen) != PolisConfig::MAGIC)) {
          $lineval = array(0 => $conf[$x]);
        }
        else {
          $lineval = explode('=',$conf[$x]);
        }
        if (substr($lineval[0],0,$magiclen) == PolisConfig::MAGIC) {
  ...
  function __construct($uname) {
    ...
    if (file_exists('/home/'.$uname.'/.poliscore/polis.conf')) {
      $this->configfilename = '/home/'.$uname.'/.poliscore/polis.conf';
    }
    elseif (file_exists('/home/'.$uname.'/.polis/polis.conf')) {
      $this->configfilename = '/home/'.$uname.'/.polis/polis.conf';
    }
  ...
  function saveconfig() {
    if ($this->configloaded) {
      ...
      foreach ($this->mnctlcfg as $key => $value) {
        if ($value === false) {
          $rawconf .= PolisConfig::MAGIC.$key."\n";
        }
        else {
          $rawconf .= PolisConfig::MAGIC.$key.'='.$value."\n";
        }
      }
