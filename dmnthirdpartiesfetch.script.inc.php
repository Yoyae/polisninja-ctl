  ...
  if (($res !== false) && is_array($res) && (count($res) == 2) && array_key_exists('return',$res)
   ...
   && is_array($res["return"]["markets"]["DRK"]) && array_key_exists("lasttradeprice",$res["return"]["markets"]["DRK"])) {
    $tp["btcdrk"] = array("StatValue" => $res["return"]["markets"]["DRK"]["lasttradeprice"],
                          "LastUpdate" => time(),
                          "Source" => "cryptsy");
    echo "OK (".$res["return"]["markets"]["DRK"]["lasttradeprice"]." BTC/POLIS)\n";
  }
  else {
    echo "Failed (JSON)\n";
  }
...
if ($res !== false) {
  $res = json_decode($res,true);
//  var_dump($res);
  if (($res !== false) && is_array($res) && (count($res) > 0) && array_key_exists('BTC_POLIS',$res)
      && is_array($res["BTC_POLIS"]) && array_key_exists("last",$res["BTC_POLIS"])) {
    $tp["btcdrk"] = array("StatValue" => $res["BTC_POLIS"]["last"],
        "LastUpdate" => time(),
        "Source" => "poloniex");
    echo "OK (".$res["BTC_POLIS"]["last"]." BTC/POLIS)\n";
  }
  else {
    echo "Failed (JSON)\n";
  }
...
// https://bittrex.com/api/v1.1/public/getticker?market=BTC-POLIS

xecho("Fetching from CoinMarketCap: ");
$res = file_get_contents('http://coinmarketcap-nexuist.rhcloud.com/api/polis');
$resdone = 0;
if ($res !== false) {
  $res = json_decode($res,true);
  if (($res !== false) && is_array($res) && array_key_exists('symbol',$res) && ($res['symbol'] == 'polis') && array_key_exists('timestamp',$res)) {
    $tbstamp = date('Y-m-d H:i:s',$res['timestamp']);
...
//TODO
xecho("Fetching budgets list from PolisCentral: ");
$res = file_get_contents('https://www.poliscentral.org/api/v1/budget?partner='.DMN_POLISWHALE_PARTNERID);
$proposals = array();
if ($res !== false) {
  $res = json_decode($res,true);
...
foreach($proposals as $proposal) {
  xecho("Fetching budget $proposal from PolisCentral: ");
  $res = file_get_contents('https://www.poliscentral.org/api/v1/proposal?partner='.DMN_POLISWHALE_PARTNERID.'&hash='.$proposal);
